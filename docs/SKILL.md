...
Azure resource security auditor.
...
```
go install github.com/ppiankov/azurespectre/cmd/azurespectre@latest
```
...
Scans Azure resources for security findings.
...
- Does not remediate or modify Azure resources — scan is read-only
- Does not store findings or manage a findings database
- Does not replace dedicated Azure resources monitoring — point-in-time security audit only
...
```bash
azurespectre scan --output json | jq '.summary'
azurespectre scan --output json | jq '.findings[] | select(.severity == "critical")'
```
